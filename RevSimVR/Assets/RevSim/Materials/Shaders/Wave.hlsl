...
float3 SineWave (float4 wave, float3 p, inout float3 tangent, inout float3 binormal) {
	...
	tangent += normalize(float3(1, d.x * steepness * cos(f), 0));
	binormal += float3(-tangent.y, tangent.x, 0);
...
void DrawWaves_half(float3 vertex_in, out float3 out_pos, out float3 out_norm, out float out_y_offset)
{
    float3 tangent = 0;
    float3 binormal = 0;
	float3 normal = 0;
	...
	if (_WaveType == 0) normal = normalize(binormal);
    else if (_WaveType == 1) normal = normalize(cross(binormal, tangent));
	out_pos = p;
	out_norm = normal;
