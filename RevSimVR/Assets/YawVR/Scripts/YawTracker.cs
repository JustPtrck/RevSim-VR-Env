...
namespace YawVR {
    ...
    public class YawTracker : MonoBehaviour {
        ...
        private Vector3 ApplyLimits(Vector3 rot) {
            float pitchFrontLimit = yawController.Limits.pitchFront;
            float pitchBackLimit = yawController.Limits.pitchBack;
            if (pitchFrontLimit != -1 && pitchBackLimit != -1)  rot.x = Mathf.Clamp(rot.x, -pitchFrontLimit, pitchBackLimit);
