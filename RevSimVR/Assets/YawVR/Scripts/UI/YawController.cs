...
namespace YawVR {
    ...
    [Serializable]
    public class Limits {
        public float yaw = -1, pitchFront = -1, pitchBack = -1, roll = -1;
       
        public Limits(float yaw, float pitchFront, float pitchBack, float roll) {
            this.yaw = yaw;
            this.pitchFront = pitchFront;
            this.pitchBack = pitchBack;
            this.roll = roll;
        }
    }
    /// <summary>
    ...
    public interface YawControllerDelegate {
        ...
        void SetBatteryPercent(float fBatteryPercent);
    ...
    public class YawController : MonoBehaviour, YawControllerType, YawTCPClientDelegate, YawUDPClientDelegate {
        ...
        public bool IsRememberDevice = false;
        public int? SavedUdpPort = 0;
        public int? SavedTcpPort = 0;
                    ...
                    if (m.Groups.Count >= 2) {
                        switch (m.Groups[1].Value) {
                            ...
                            case "U":
                                ...
                                ControllerDelegate?.SetBatteryPercent(device.batteryPercent);
                                break;
        ...
        public void DidRecieveTCPMessage(byte[] data) {
            ...
            switch (commandId) {
                ...
                case CommandIds.GET_TEMPS:
                    ...
                    int nTemperature = BitConverter.ToInt32(device.temps, 0);
                    ControllerDelegate?.SetTemperature(nTemperature);
        ...
        public void SetTiltLimits(float yawLimit, float pitchLimit, float rollLimit) {
            ...
            gameLimits.pitchFront = gameLimits.pitchBack = pitchLimit;
        ...
        public void SetTiltLimits(float yawLimit, float pitchFrontLimit, float pitchBackLimit, float rollLimit)
        {
            gameLimits.yaw = yawLimit;
            gameLimits.pitchFront = pitchFrontLimit;
            gameLimits.pitchBack = pitchBackLimit;
            gameLimits.roll = rollLimit;

            int pitchFrontMax = (int)pitchFrontLimit;
            int pitchBackMax = (int)pitchBackLimit;
            int rollMax = (int)rollLimit;
            tcpCLient.BeginSend(Commands.SET_TILT_LIMITS(pitchFrontMax, pitchBackMax, rollMax));
